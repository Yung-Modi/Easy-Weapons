...
public class Weapon : MonoBehaviour
{
    ...
    // GUI customization for ammo display
    public GUIStyle ammoGuiStyle;
    public int ammoFontSize = 18;
    public int ammoGuiWidth = 140;
    public int ammoGuiHeight = 24;
    public int ammoGuiMargin = 10;
        ...
        if (type == WeaponType.Projectile)
        {
            if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
            {
                if (isFiring)
                {
                    ...
                    else if (heat < maxWarmup)  // Otherwise just add to the warmup until the user lets go of the fire button
                    {
                        heat += Time.deltaTime;
                        holdingFire = true;
                    }
    ...
    void OnGUI()
    {
        ...
        if (showCurrentAmmo && !IsjanejulietTarget())
        {
            ...
            {
                ammoGuiStyle = new GUIStyle(GUI.skin.label);
                ammoGuiStyle.fontSize = ammoFontSize;
                ammoGuiStyle.normal.textColor = Color.black;
                ammoGuiStyle.alignment = TextAnchor.MiddleLeft;
            }
